...
def plot(path: str, reps: int, levels: list[int], period_adj: float):
    ...
    blocks = [me[reps*sum(levels[:i]): reps*sum(levels[:i+1])]
              for i in range(0, 4)]
    ...
    ratings = [np.sum([blocks[i][j, :, -1] for j in range(reps)],
                      axis=0) / reps for i in range(len(blocks))]
    ...
    errors = [[np.std(blocks[i][:, j, 5]) / np.sqrt(reps)
               for j in range(levels[i])] for i in range(len(blocks))]

    keys = [blocks[i][0, :, i+1] for i in range(len(blocks))]

    # period adjustment
    keys[3] = [i * period_adj for i in keys[3]]

    plt.rcParams['figure.figsize'] = [10, 8]
    fig, axs = plt.subplots(2, 2)
    for i in range(len(blocks)):
        axs[i//2, i % 2].errorbar(keys[i], ratings[i],
                                  errors[i], color="black", capsize=5)
...
plot("data/ss2022-07-1511-06-13.775862.csv", reps=10,
     levels=[10, 10, 6, 10], period_adj=10/9)
plot("data/me.csv", reps=3, levels=[10, 10, 6, 10], period_adj=10/9)
plot("data/ec2022-07-1513-01-53.549794.csv", reps=10,
     levels=[10, 10, 6, 10], period_adj=10/9)
